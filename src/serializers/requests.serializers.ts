...
import {
  ...
  ICreateProductToRequestResponse,
  IUpdateRequest,
...
const createRequestResponseSerializer: SchemaOf<ICreateRequestResponse> = yup
  .object()
  .shape({
...
const createProductToRequestsResponseSerializer: SchemaOf<ICreateProductToRequestResponse> =
  yup.object().shape({
    productName: yup.string(),
    quantity: yup.number(),
    value: yup.number(),
...
const listRequestByIdResponseSerializer: SchemaOf<IListRequestResponse> = yup
  .object()
  .shape({
